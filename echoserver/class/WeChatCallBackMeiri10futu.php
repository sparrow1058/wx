...
class WeChatCallBackMeiri10futu extends WeChatCallBack{
	...
	public function process(){
		...
		if($this->_content=='q'||$this->_content=='Q'||$this->_content=='?'||$this->_content=='？'){
			...
			interface_log(DEBUG,0,'get next url ret:'.var_export(
			$ret,true));
			if($ret['code']){
		...
		$resultStr=sprintf(SUCC_TPL_MR,$this->_fromUserName,$this->_toUserName,
				$this->_time,$this->_index,$url,$url);
		return $resultStr;
	}
	...
	private function _getPicUrlByIndex($index){
		$fileName=dirname (__FILE__)."/../image/".$index.".gif";
		...
		if(file_exists($fileName)){
			...
			return array('data'=>IMAGE_URL.$index.".gif",'code'=>0);
		}else{
	...
	private function _addSeen(){
		...
		try{
			...
			if($seen>PIC_OF_DAY&&$limited==1){
				...
				// user not limited or $seen pics not too many
